...
def get_gemini_response(prompt):
    """Fetch and format response from Gemini AI, ensuring it ends at a full stop when words are between 40-50."""
    try:
...
def main(text):
		...
		if result != "None": speak(result, True, True)
			...
			speak(ai_response, True , True)			#speak("Here's what I found on the web... ", True, True)
...
def showSingleImage(type, data=None):
	...
	try:
		img0 = ImageTk.PhotoImage(Image.open('Downloads/1.jpg').resize((90,110), Image.Resampling.LANCZOS))
	except:
		pass
	...
	if type=="weather":
		...
		img3 = ImageTk.PhotoImage(Image.open('extrafiles/images/dice/'+type+'.jpg').resize((200,200), Image.Resampling.LANCZOS))
...
def showImages(query):
	...
	img4 = ImageTk.PhotoImage(Image.open('Downloads/4.jpg').resize((w,h), Image.Resampling.LANCZOS))
	#Displaying
